...
def end_round_callback(gid):
    socketio.emit("game_state_update", {"isNewRound": False, "message": "Round has ended! Collecting lists and calculating scores...", "game": games[str(gid)].encode()}, room = str(gid))
...
def create_game(gid, height, width, min_letters, minutes, language):
    ...
    games[gid] = Game(gid, GameProperties(min_letters = min_letters, minutes = minutes), grid, analyzer, end_round_callback, list_request_callback, send_analysis_callback)
...
def handle_game_join_event(json, methods = ["GET", "POST"]):
    ...
    if games[json["gid"]].add_player(json["username"]):        
        join_room(json["gid"])
        emit("game_state_update", {"isNewRound": False, "message": "{0} has joined the room!".format(json["username"]), "game": games[json["gid"]].encode()}, room = str(json["gid"]))
    else:
